...
class SimpleReport:
    @classmethod
    def find_next_date(self, products_list):
    ...
    def generate(self, products_list):
        report = ""
        oldest = min(products_list, key=itemgetter("data_de_fabricacao"))
        oldest = oldest["data_de_fabricacao"]
        report += f"Data de fabricação mais antiga: {oldest}\n"
        nearest = self.find_next_date(products_list)
        report += (
